...
- [x] 0. 구조 설계 — 파일 트리, 루트 문서, 에이전트, CONTRACT/COLOR_SPEC (2026-09-22 커밋 3831d4a)
...
- 진행 순서: 팔레트 → 조화 규칙 → 얼굴 제약. 앞 파일이 확정되기 전에 다음으로 넘어가지 않는다
- [x] 팔레트 질문 해소: 색 수 45, 중성색 최소 N=12, 태그 `neutral` 단일, Lab 출처 hex 계산, 허용 오차 ΔE76 0.05, 무채색 임계 C*<5
- [ ] 조화 규칙 질문 해소: hue 구간 표현(부호 유무), 전략별 목표 구역, 채도 관계, 랭킹 가중치, 서브/포인트 선정 규칙
- [ ] 얼굴 제약 질문 해소: ΔL 하한, 피부 색상대 회피 범위와 채도 임계, 저채도 임계, 피부–립 구분 불가 임계
...
- [x] assets/rules/palette_v1.json — 45색 (neutral 16). 로더 검사 11항목 통과 (RULES_SCHEMA §2.4)
- [~] docs/RULES_SCHEMA.md — 팔레트 절(§2)·명명 절(§1 팔레트 id 확정) 작성. harmony·face_constraints 절은 해당 파일과 함께
- [~] 식별자 명명 규칙 (RULES_SCHEMA §1): 팔레트 id 확정. 규칙 ID / 진단 식별자 / 에러 코드는 harmony_rules 작성 시 확정 (진단·에러 코드는 잠정 규칙 사용 중).
      조화 구간값이 먼셀 기준 원 이론과 다른 CIELCh 초기 추정치라는 주의는 harmony 절 파라미터 설명에 적는다 (COLOR_SPEC §2)
...
### 1-6. 뷰어 (prototype/)
- [x] prototype/palette.html — 팔레트 스와치 (2026-09-22). 결과 뷰어 본체(index.html)와 **별개 파일**.
      palette_v1.json을 fetch해 격자(정렬: 계열별 h° / 명도순 L* / neutral 먼저), 배경 전환(피부 L* 78/62/45/30, 흰, 회), L* 분포 축.
      색 계산은 저장된 Lab의 C*/h° 극좌표 표기만 (CONTRACT §3 예외). 2단계 검수와 색 추가 시에도 쓴다
- [ ] prototype/index.html — 결과 뷰어 본체 (아래 항목)
...
| 팔레트 v1 | 45색(neutral 16). 태그 `neutral` 단일 + `tagVocabulary`. Lab은 hex에서 계산해 소수 2자리. 로더 검사: hex↔lab ΔE76 ≤ 0.05, neutral ≥ 12, neutral C* ≤ 35. 명도대 보장 검사는 넣지 않음(런타임 no_candidate와 중복) (RULES_SCHEMA §2) |
| 무채색 임계 | C* < 5, harmony_rules에 둔다 (COLOR_SPEC §1.4) |
| 저채도 임계 | face_constraints에서 정한다. 팔레트 단계에서는 소비자가 없어 이연 |
| 팔레트 id 규칙 | `^[a-z][a-z0-9]*(_[a-z0-9]+)*$`, 영문 의류 색 이름 snake_case (RULES_SCHEMA §1) |
...
팔레트 — 전부 해소 (2026-09-22, RULES_SCHEMA §2.6)
- ~~색 개수~~ → 45. ~~중성색 최소 N~~ → 12. ~~계열 태그~~ → `neutral` 단일. ~~Lab 값 출처~~ → hex에서 계산, 소수 2자리

팔레트 — 규칙 적용 후 확인 (스와치 육안 검토 2026-09-22. 지금 고치지 않고 `no_candidate` 진단과 실제 결과를 보고 판단)
- 퍼플 계열: 라벤더(L* 73)와 퍼플/grape(L* 39) 사이 34의 공백. orchid(L* 58) 추가로 메웠으나 충분한지
- 로즈 핑크(L* 57, C* 40, h° 3)와 모브(L* 55, C* 18, h° 343): 명도가 거의 같고 색상도 20° 차이, 채도만 다르다.
  둘 다 실제로 쓰이는지, 아니면 하나가 늘 탈락하는지
- 레드 계열: 셋 다 어둡다(L* 43/39/25). 블러시(L* 82)와 브릭(L* 39) 사이가 비어 보인다. 밝은 레드 후보가 필요한지
